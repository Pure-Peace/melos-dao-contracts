    ...
    ERC20PermitUpgradeable
{
    ...
    /**
     * @dev Retrieve the `voters` at the end of `blockNumber`.
     */
    function getPastVoters(uint256 blockNumber) public view virtual override returns (uint256) {
        require(blockNumber < block.number, "ERC20Votes: block not yet mined");
        return _checkpointsLookup(_votersCheckpoints, blockNumber);
    }
    ...
    function _mint(address account, uint256 amount) internal virtual override(ERC20Upgradeable) {
        ...
        _updateVoters(oldBalance, oldBalance + amount);
    }
    ...
    function _moveVotingPower(
        ...
        if (src != dst && amount > 0) {
            if (src != address(0)) {
                ...
                _updateVoters(oldWeight, newWeight);
    ...
    function _updateVoters(uint256 oldBalance, uint256 newBlance) internal {
